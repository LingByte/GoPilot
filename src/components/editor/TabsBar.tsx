...
export default function TabsBar({ tabs, activeId, onActivate, onClose, onContextMenu }: TabsBarProps) {
  return (
    <div
      className="h-10 flex items-center border-b border-gray-200 bg-white overflow-x-auto"
      onContextMenu={(e) => {
        // empty area context menu
        if (!onContextMenu) return;
        // If user right-clicked on a tab item, that handler will run and stopPropagation.
        e.preventDefault();
